...
if _getgenv then
	local ok, result = pcall(function()
		return _getgenv().DISABLE_RAYFIELD_REQUESTS
	end)
...
local function secureNotify(wType, title, content)
	if secureWarnings[wType] then
		return
	end
	secureWarnings[wType] = true
	task.spawn(function()
		while not RayfieldLibrary or not RayfieldLibrary.Notify do
			task.wait(0.5)
		end
...
local function loadSettings()
	...
	{"General":{"rayfieldOpen":{"Value":"K","Type":"bind","Name":"Rayfield Keybind","Element":{"HoldToInteract":false,"Ext":true,"Name":"Rayfield Keybind","Set":null,"CallOnChange":true,"Callback":null,"CurrentKeybind":"K"}}},"System":{"usageAnalytics":{"Value":false,"Type":"toggle","Name":"Anonymised Analytics","Element":{"Ext":true,"Name":"Anonymised Analytics","Set":null,"CurrentValue":false,"Callback":null}}}}
		...
		if file then
			local decodeSuccess, decodedFile = pcall(function()
...
if not requestsDisabled and not useStudio then
	local fetchSuccess, fetchResult = pcall(
		(game :: any).HttpGet,
		game,
		"https://raw.githubusercontent.com/SiriusSoftwareLtd/Rayfield/refs/heads/main/reporter.lua"
	)
...
repeat
	...
	toDestroy, Rayfield =
		Rayfield,
		useStudio and script.Parent:FindFirstChild("Rayfield") or game:GetObjects("rbxassetid://" .. RayfieldAssetId)[1]
...
local function resolveIcon(icon)
	...
	if secureMode then
		secureNotify(
			"icon_blocked",
			"Secure Mode",
			"Element icons using asset IDs or Lucide names are blocked. Use getcustomasset() for icons to stay undetected."
		)
		return "", nil, nil
	end
...
local function spawnNotification(container, data)
	task.spawn(function()
		...
		if data.Image then
			...
			if rectOffset then
				newNotification.Icon.ImageRectOffset = rectOffset
			end
			if rectSize then
				newNotification.Icon.ImageRectSize = rectSize
			end
		else
					...
					if element.Name ~= "SectionSpacing" and element.Name ~= "Placeholder" then
						...
						then
							TweenService:Create(
								element.Title,
								TweenInfo.new(0.3, Enum.EasingStyle.Exponential),
								{ TextTransparency = show and 0.4 or 1 }
							):Play()
						elseif element.Name == "Divider" then
							TweenService:Create(
								element.Divider,
								TweenInfo.new(0.3, Enum.EasingStyle.Exponential),
								{ BackgroundTransparency = show and 0.85 or 1 }
							):Play()
						else
							TweenService:Create(
								element,
...
local function Hide(notify: boolean?)
	...
	if notify then
		if useMobilePrompt then
			...
			RayfieldLibrary:Notify({
				Title = "Interface Hidden",
				Content = "The interface has been hidden, you can unhide the interface by tapping " .. tostring(
					getSetting("General", "rayfieldOpen")
				) .. ".",
				Duration = 7,
				Image = 4400697855,
			})
...
function RayfieldLibrary:CreateWindow(Settings)
	...
	if Settings.ToggleUIKeybind then -- Can either be a string or an Enum.KeyCode
		...
		if type(keybind) == "string" then
			...
			assert(
				...
				"ToggleUIKeybind must be a valid KeyCode"
			)
	...
	if Settings.Icon and Settings.Icon ~= 0 and Topbar:FindFirstChild("Icon") then
		...
		if Settings.Icon then
			...
			if rectOffset then
				Topbar.Icon.ImageRectOffset = rectOffset
			end
			if rectSize then
				Topbar.Icon.ImageRectSize = rectSize
			end
		else
			Topbar.Icon.Image = ""
		end
	...
	if Settings.KeySystem then
		...
		if not Passthrough and secureMode then
			warn(
				"Rayfield | Secure Mode: Key system requires a valid saved key. The key UI cannot be shown as it requires loading detectable assets."
			)
			Rayfield.Enabled = false
	...
	function Window:CreateTab(Name, Image, Ext)
		...
		if Image and Image ~= 0 then
			...
			if rectOffset then
				TabButton.Image.ImageRectOffset = rectOffset
			end
			if rectSize then
				TabButton.Image.ImageRectSize = rectSize
			end
		...
		function Tab:CreateColorPicker(ColorPickerSettings) -- by Throit
			...
			local colorPickerInputConnection = UserInputService.InputEnded:Connect(function(input, gameProcessed)
				if
		...
		function Tab:CreateLabel(LabelText: string, Icon: number, Color: Color3, IgnoreTheme: boolean)
			...
			if Icon then
				...
				if rectOffset then
					Label.Icon.ImageRectOffset = rectOffset
				end
				if rectSize then
					Label.Icon.ImageRectSize = rectSize
				end
			else
				Label.Icon.Image = ""
			end
		...
		function Tab:CreateSlider(SliderSettings)
			...
			Slider.Main.Progress.Size = UDim2.new(
				...
				Slider.Main.AbsoluteSize.X
							...
							> 5
						and Slider.Main.AbsoluteSize.X * ((SliderSettings.CurrentValue - SliderSettings.Range[1]) / (SliderSettings.Range[2] - SliderSettings.Range[1]))
					or 5,
				1,
				0
			)
						...
						TweenInfo.new(0.45, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out),
						{
							Size = UDim2.new(
								...
								Slider.Main.AbsoluteSize.X
											...
											> 5
										and Slider.Main.AbsoluteSize.X * ((NewVal - SliderSettings.Range[1]) / (SliderSettings.Range[2] - SliderSettings.Range[1]))
									or 5,
								1,
								0
							),
	...
	if reporter and getSetting("System", "usageAnalytics") then
		...
		if
			...
			and Settings.Discord.Invite
			and Settings.Discord.Invite ~= ""
		then
			local raw = tostring(Settings.Discord.Invite)
			-- Normalize: strip URL prefixes to extract just the invite code
			discordInvite = (raw:match("discord%.gg/([%w%-]+)") or raw:match("discord%.com/invite/([%w%-]+)") or raw):sub(
				1,
				32
			)
		end
